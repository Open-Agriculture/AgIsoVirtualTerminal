...
void ObjectPointerComponent::on_content_changed(bool initial)
{
	...
	if (nullptr != child)
	{
		...
		if (nullptr != childComponent)
		{
			...
			// Components size themselves, so a chain of object pointers ends up as large as the object it resolves to
			setSize(childComponent->getWidth(), childComponent->getHeight());
		}
	}
